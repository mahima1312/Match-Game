...
class App extends Component {
  ...
  countDown = () => {
    this.setState(prevState => {
      if (prevState.timer === 1) {
        clearInterval(this.timerId)
        return {isGameOver: true, timer: 0}
      }
  ...
  renderMatchImage = () => {
    const {currentImage, activeTabId} = this.state
    const activeTabIdImagesList = this.getActiveTabIdImages()
      ...
      <div className="match-game-container">
        <div className="images-container">
          ...
          <ul className="thumbnail-images">
            {activeTabIdImagesList.map(eachImage => (
              <ThumbnailItem
                key={eachImage.id}
                categoryDetails={eachImage}
  ...
  getActiveTabIdImages = () => {
    const {activeTabId} = this.state
    return imagesList.filter(eachImage =>
  ...
  render() {
    ...
    return (
      <div className="app-container">
        <Header score={score} timer={this.formatTimer()} />
        {isGameOver ? (
